...
before(function () {
	...
	IoC.factory("depB.depC", require("./injectables/setter_annotated/nestedDeps/depX"));

});

describe('nestedDeps2_setter', function () {
	it('instancesEquality', function () {
		expect(IoC.create("depA").depB.depC).to.eql(IoC.create("depB.depC"));
		expect(IoC.create("depB").depC).to.eql(IoC.create("depB.depC"));
